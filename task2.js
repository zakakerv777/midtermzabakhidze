...
const students = [
    ...
    // თქვენი კოდი აქ
      return students
          .map(student => {
              const grades = Object.values(student.grades);
              const average = grades.reduce((sum, grade) => sum + grade, 0) / grades.length;
              return { name: `${student.firstName} ${student.lastName}`, average };
          })
          .filter(student => student.average > gradeThreshold)
  ...
  function getMultiAchivers(students, threshold) {
    // თქვენი კოდი აქ
    return students.map(student => {
      const highAchievingSubjects = Object.values(student.grades).filter(grade => grade > threshold);
      return {
          name: `${student.firstName} ${student.lastName}`,
          expertSubjects: highAchievingSubjects.length
      };
